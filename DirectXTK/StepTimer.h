...
#include <cmath>
#include <cstdint>
// #. pch.h에서 Windows.h를 상단에 놓고 StepTimer.h를 하단에 놓으면 StepTimer.h에서 Windows.h를 인식한다.
    ...
    public:
        ...
        void Tick(const TUpdate& update)
        {
            ...
            if (m_isFixedTimeStep)
            {
                ...
                while (m_leftOverTicks >= m_targetElapsedTicks) // 고정 시간( While 문으로 가둬놓음 )
                {
            ...
            if (m_qpcSecondCounter >=
                static_cast<uint64_t>(m_qpcFrequency.QuadPart)
                )
            {
                m_framesPerSecond = m_framesThisSecond;
                m_framesThisSecond = 0;
                m_qpcSecondCounter %= static_cast<uint64_t>(
